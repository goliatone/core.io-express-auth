...
module.exports = function(app, config) {
    ...
    passport.endpoint = function(req, res, next) {
        ...
        if (!strategies.hasOwnProperty(provider)) {
            ...
            if (process.env.NODE_ENV === 'production') {
                return res.redirect(config.passport.failureRedirect);
            }
    ...
    passport.connect = function(req, query, profile, next) {
        ...
        config.logger.info('auth profile', query, profile);
        ...
        Passport.findOne({
            ...
            if (!req.user) {
                ...
                if (!passport) {
                    /*
                     * In the case that a user wants to assing
                     * their profile with multiple passports, 
                     */
                    return PassportUser.findOrCreate(user).then(record => {
                        user = record;
                        query.user = user.id;
