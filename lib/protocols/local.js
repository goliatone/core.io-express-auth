        ...
        PassportUser.findOne(query).then((user) => {
            if (!user) {
                ...
                if (isEmail) {
                    ...
                    setFlash(req, 'Error.Passport.Email.NotFound');
                } else {
                    message = 'Username not found'
                    setFlash(req, 'Error.Passport.Username.NotFound');
                }
            ...
            return Passport.findOne({
                ...
                if (passport) {
                    return passport.validatePassword(password).then((res) => {
                        if (!res) {
                            setFlash(res, 'Error.Passport.Password.Wrong');
                            return next(null, false);
                        }
                        return next(null, user, passport);
                    }).catch(next);
                }
                setFlash(res, 'Error.Passport.Password.NotSet');
